...
class Socket implements MessageComponentInterface {
    ...
    public function onOpen(ConnectionInterface $interface) {
        ...
        $broadcast = array(
			...
			'action' 	=> 'chat',
			'chat'		=> array(
				'from'	=> 'Nama Pengirim '.$interface->resourceId,
				'msg'	=> 'Test Pesan '.rand(),
			),
			'button'	=> array(
				'play'	=> false,
			...
			'watch'		=> array(
				'name'	=> 'Name who enter room '.rand(),
				'ava'	=> 'https://i.ytimg.com/vi/0EnrFe3Zb6k/maxresdefault.jpg?id='.rand(),
			),
        );

        $broadcastWatch = array(
            // chat, button, watch, message
            'action'    => 'watch',
            'chat'      => array(
                'from'  => 'Nama Pengirim '.$interface->resourceId,
        ...
        foreach ($this->ws_client as $client) {
            $client->send(json_encode($broadcast));
            $client->send(json_encode($broadcastWatch));
            $client->send(json_encode($broadcastButton));
        }
                ...
                if(count($check_game) > 0) {
                    ...
                    if(count($check_user_token) > 0) {
                        ...
                        if($exec) {
                            ...
                            // SELF
                            $this->ws_client[$interface->resourceId]->send(json_encode($broadcastButtonSelf));
                            ...
                            if(count($room_data) > 0) {
                                ...
                                foreach ($room_data as $keyRoom => $valRoom) {
                                    foreach ($check_user_token as $keyUser => $valUser) {
                                        ...
                                        $this->ws_client[$valRoom->id_ws]->send(json_encode($broadcastWatch));
                                    }
                                }
                            } else {
                            ...
                            foreach ($list_user_in_room as $keyRoom => $valRoom) {
                                ...
                                if(count($user_token) > 0) {
                                    foreach ($user_token as $keyUsr => $valUsr) {
                                        ...
                                        $this->ws_client[$valRoom->id_ws]->send(json_encode($broadcastChat));
                                    }
                                } else {
                                    ...
                                    if(count($user_token) > 0) {
                                        ...
                                        foreach ($user_token as $keyUsr => $valUsr) {
                                            if($valRoom->id_ws != $interface->resourceId) {
                                                ...
                                                // BROADCAST
                                                $this->ws_client[$valRoom->id_ws]->send(json_encode($broadcastButton));
                                            }
                                        }
                                    } else {
